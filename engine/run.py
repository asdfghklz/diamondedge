...
PARK_FACTORS = {k: (v["factor"] if isinstance(v, dict) else v) for k, v in LOOKUP["park_factors"].items()}
TEAM_HOME_WR = {k: (v["wr"] if isinstance(v, dict) else v) for k, v in LOOKUP["team_home_wr"].items()}
TEAM_AWAY_WR = {k: (v["wr"] if isinstance(v, dict) else v) for k, v in LOOKUP["team_away_wr"].items()}
UMP_BIAS     = {k: (v["bias"] if isinstance(v, dict) else v) for k, v in LOOKUP["ump_bias"].items()}
...
def main():
    ...
    # Run first inning predictions
    print("\n[F1] Running first inning predictions...")
    f1_picks = run_f1_predictions(games, date_str)
    # Preserve existing F1 results
    if existing_picks:
        f1_results_map = {fp["game_id"]: fp.get("result") for fp in existing_data.get("f1_picks",[]) if fp.get("result")}
        for fp in f1_picks:
            if fp["game_id"] in f1_results_map:
                fp["result"] = f1_results_map[fp["game_id"]]
    output["f1_picks"] = f1_picks
    output["f1_confident_count"] = len([p for p in f1_picks if p["prediction"]])
...
def get_team_f1_stats(team_id, n_games=20):
    ...
    return {"scored_pct":round(sum(s)/len(s),3),"allowed_pct":round(sum(a)/len(a),3),"recent_scored":scored_f1[-5:],"recent_allowed":allowed_f1[-5:],"games":len(s),"provisional":False}

def get_sp_f1_rate(pitcher_id):
    """Estimate SP first inning RA rate from ERA (provisional) or game logs."""
    if not pitcher_id:
    ...
    except: era,gs,ip = 4.50,0,0
    if gs < 3:
        return {"f1_ra_pct":0.40,"starts":gs,"provisional":True}
    # Calibrated: ERA 1.50 → ~18% F1 RA, ERA 3.00 → ~30%, ERA 5.00 → ~48%, ERA 7.00 → ~62%
    f1_rate = max(0.10, min(0.70, 0.18 + (era - 1.50) * 0.072))
    return {"f1_ra_pct":round(f1_rate,3),"era":era,"starts":gs,"ip":ip,"provisional":False}
        ...
        for m in markets:
            ...
            if (away_abbr in ticker or home_abbr in ticker or away_abbr in title or home_abbr in title):
                if "NRFI" in ticker or "NRFI" in title or "FIRST INNING" in title:
                    yes_p = m.get("yes_ask") or m.get("last_price")
                    no_p  = m.get("no_ask")
...
def predict_first_inning(game, date_str):
    ...
    print(f"  F1: {away_abbr}@{home_abbr} | {sp_away_name} vs {sp_home_name}")
    ...
    # Recency weighting for last 5 games (most recent = higher weight)
    def recency_score(arr):
        if not arr: return 0.40
        weights=[1,1.5,2,2.5,3]; wt=sum(weights[-len(arr):]); ws=sum(weights[-len(arr):][i]*v for i,v in enumerate(arr))
        return ws/wt if wt>0 else 0.40

    a_recent=recency_score(a_f1["recent_scored"]); h_recent=recency_score(h_f1["recent_scored"])
    a_lineup_boost=(a_hit["avg_ops"]-0.720)*0.12; h_lineup_boost=(h_hit["avg_ops"]-0.720)*0.12

    # Away scores top 1st (vs home SP)
    away_p=0.40*a_f1["scored_pct"]+0.35*hsp["f1_ra_pct"]+0.15*a_recent+0.10*max(0,min(1,0.40+a_lineup_boost))
    # Home scores bottom 1st (vs away SP)
    home_p=0.40*h_f1["scored_pct"]+0.35*asp["f1_ra_pct"]+0.15*h_recent+0.10*max(0,min(1,0.40+h_lineup_boost))

    yrfi_p=round(max(0.15,min(0.90,1-(1-away_p)*(1-home_p))),3)
    nrfi_p=round(1-yrfi_p,3)

    # NRFI guard: reject if either SP has >50% F1 RA rate
    ...
    if nrfi_p>=0.68 and not nrfi_disq:
    ...
    return {
        "game_id":f"{date_str}-F1-{away_abbr}-{home_abbr}",
        "game_pk":game_pk,"home":home_abbr,"away":away_abbr,
        "home_full":home_name,"away_full":away_name,
        "home_sp":sp_home_name,"away_sp":sp_away_name,
        "start_time":game_time,"status":"scheduled","score":"Scheduled","result":None,
        "prediction":prediction,"pick_label":pick_label,"confidence":confidence,
        "yrfi_p":yrfi_p,"nrfi_p":nrfi_p,
        "away_score_p":round(away_p,3),"home_score_p":round(home_p,3),
        "kalshi_yes":kalshi_yes,"kalshi_no":kalshi_no,"kalshi_note":kalshi_note,
        "data":{
            "away_f1_score_pct":a_f1["scored_pct"],"away_f1_allow_pct":a_f1["allowed_pct"],
