...
EV_MIN          = 0.080   # minimum 8% edge (calibrated from 33k game data)
...
# Full team name mappings for Odds API matching
# The Odds API uses full names; MLB API uses abbreviations
TEAM_FULL_NAMES = {
...
def get_pitcher_stats(pitcher_id, days=30):
    ...
    if len(splits) >= 2:
        ...
        # Require minimum 25 IP to trust last-30d ERA
        # Small samples (1-2 starts) are too noisy and cause bad predictions
        if ip >= 25:
            era  = round((er/ip)*9, 2)
...
def get_odds():
    ...
    missing = []
    print(f"  Odds: {len(result)} games | Books: FanDuel, BetMGM")
...
def run_model(game, odds_map, date_str):
    ...
    prop_adj = team_delta * 0.50  # calibrated from historical data
    ...
    # Try multiple key formats — Odds API team names sometimes differ from MLB API
    home_full = TEAM_FULL_NAMES.get(home_abbr, home_name)
    away_full = TEAM_FULL_NAMES.get(away_abbr, away_name)
    game_lines = (odds_map.get(f"{home_full}|{away_full}") or
                  odds_map.get(f"{home_name}|{away_name}") or
                  odds_map.get(f"{home_name}|{away_full}") or
                  odds_map.get(f"{home_full}|{away_name}") or {})
    ...
    output = {
        ...
        "model": {
            ...
            "data_sources": [
                ...
                "FanDuel + BetMGM via The Odds API",
